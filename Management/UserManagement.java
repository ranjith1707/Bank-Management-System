	...
	public int getAge(String age) {
	 ...
	 protected void transactionHistory(long accountNumber) {
			   ...
			   if(!BankDatabase.getDatabase().getTransactionHistory().isEmpty()) {
				   System.out.printf("%15s %15s %20s %20s %34s %20s","From","To"," Time","Date","Transaction Amount","status");
				   System.out.println();
			   ...
			   for(Transactions transactionHistory: BankDatabase.getDatabase().getTransactionHistory()) {
				   ...
				   System.out.printf("%20s|%18s|%20s|%20s|%20s|%25s",transactionHistory.getAccountNumber(),transactionHistory.getReciverAccountNumber(),transactionHistory.getTime(),transactionHistory.getDate(),transactionHistory.getAmount(),transactionHistory.getStatus());
				   System.out.println();
